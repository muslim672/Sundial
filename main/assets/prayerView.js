let stampEl = document.getElementById('stamp');
let diffD = 0;
...
let elevEl = document.getElementById('elev');
...
let Interval = 50;
setInterval(setPrayerTimes, Interval);
...
let prayerTableEl = document.getElementById('prayerTable');
...
function Day(n = 0) {
    let i = 0;
    for (let prayerTime of getPrayerTimes(n)) {
        if (prayerTime > stamp) {
            return [prayerTime, i];
        }
        i++;
